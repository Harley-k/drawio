...
//			}
  ...
  function getStrokeStyle(properties) {
    ...
    } else if (properties.StrokeStyle != null && properties.StrokeStyle.substring(0, 6) == 'dashed') {
  ...
  function addCustomData(cell, p, graph) {
    ...
    for (var property in p) {
      if (p.hasOwnProperty(property) &&
        ...
        try {
          var data = p[property];
          var key = mxUtils.trim(data.Label).replace(/[^a-z0-9]+/ig, '_').replace(/^\d+/, '').replace(/_+$/, '');
      ...
      if (p != null) {
        ...
        if (cell.edge) {
          ...
          } else if (isCurved) //Curved with the default waypoints
          {
            ...
            //TODO This is best-effort approximation (close enouhh but not exact)
            waypoints.push({
              x: p.Endpoint1.x + (p.Endpoint1.LinkX < 0.1 ? -250 : (p.Endpoint1.LinkX > 0.9 ? 250 : 0)),
              y: p.Endpoint1.y + (p.Endpoint1.LinkY < 0.1 ? -250 : (p.Endpoint1.LinkY > 0.9 ? 250 : 0))
            });
            waypoints.push({
              x: p.Endpoint2.x + (p.Endpoint2.LinkX < 0.1 ? -250 : (p.Endpoint2.LinkX > 0.9 ? 250 : 0)),
  ...
  function addCompositeShape(obj, select, graph) {
	...
	*/
      ...
      case 'UI2TableBlock' :
        ...
        try {
          var fillClr = getColor(p.FillColor), lineClr = getColor(p.LineColor), header, altRow, borderStyle = '',
            rowH = 20;
...
//				break;
  ...
  function createOrgChart(objId, props, data, graph, lookup) {
    ...
    try {
      ...
      var txtStyles = [], marginW = 25, marginH = 40, imgSize = 54, hasImage = true,
        cellStyle = addAllStyles('', cellDefaultStyle.props, {}, chartGroup, true);
